...
def generate_diagnostics(df, target_species, output_dir, bin_size=0.1, sampling_metadata=None):
    """
    ...
    Args:
        ...
        sampling_metadata: Optional DataFrame with columns [species, confidence, device_id, confidence_bin]
                          tracking which species each segment was specifically sampled for
    """
    ...
    if sampling_metadata is not None and not sampling_metadata.empty:
        ...
        _plot_sampled_for_species_confidence(sampling_metadata, target_species, bin_labels, output_path)
    ...
    if not expanded_df.empty:
        ...
        _plot_species_by_confidence(expanded_df, target_species, bin_labels, output_path, suffix="_all_species")
        _plot_site_by_confidence(expanded_df, bin_labels, output_path, suffix="_all_species")
        _plot_species_site_matrix(expanded_df, target_species, output_path, suffix="_all_species")
        _plot_confidence_distribution(expanded_df, output_path)

    print(f"  ✓ Saved diagnostic plots to {output_path}/")


def _plot_sampled_for_species_confidence(metadata, target_species, bin_labels, output_path):
    ...
    pivot = metadata.groupby(["species", "confidence_bin"], observed=True).size().unstack(fill_value=0)
...
def _plot_sampled_for_site_confidence(metadata, bin_labels, output_path):
    ...
    pivot = metadata.groupby(["device_id", "confidence_bin"], observed=True).size().unstack(fill_value=0)
...
def _plot_sampled_for_species_site(metadata, target_species, output_path):
    ...
    fig, ax = plt.subplots(figsize=(max(8, len(sites) * 1.2), max(6, len(target_species) * 0.5)))

    pivot = metadata.groupby(["species", "device_id"], observed=True).size().unstack(fill_value=0)
...
def _plot_species_by_confidence(df, target_species, bin_labels, output_path, suffix=""):
    ...
    # Create pivot table
    pivot = df.groupby(["species", "conf_bin"], observed=True).size().unstack(fill_value=0)
...
def _plot_site_by_confidence(df, bin_labels, output_path, suffix=""):
    ...
    pivot = df.groupby(["device_id", "conf_bin"], observed=True).size().unstack(fill_value=0)
...
def _plot_species_site_matrix(df, target_species, output_path, suffix=""):
    ...
    pivot = df.groupby(["species", "device_id"], observed=True).size().unstack(fill_value=0)
...
def _plot_confidence_distribution(df, output_path):
    ...
    ax.axvline(df["confidence"].median(), color="orange", linestyle="--", label=f'Median: {df["confidence"].median():.2f}')
